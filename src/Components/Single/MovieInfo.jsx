...
function MovieInfo() {
  ...
  return (
    ...
    <div className='w-full xl:h-screen md:h-[50vh] xs:h-[50vh] relative text-white'>
      <img
        src={`http://localhost:3000/images/films/${movie.poster}`}
        alt={movie.title}
        className='w-full h-full xl:inline-block md:inline-block xs:inline-block object-cover'
      />
      <div className="xl:bg-main md:bg-main flex-colo xl:bg-opacity-90 xl:absolute md:bg-opacity-90 md:absolute top-0 right-0 bottom-0 left-0">
        <div className="container px-3 mx-auto 2xl:px-32 xl:grid md:grid grid-cols-3 flex-colo py-5 lg:py-10 gap-8">
          ...
          <div className="col-span-2 md:grid grid-cols-5 gap-4 items-center">
            <div className="col-span-3 flex flex-col gap-10">
              ...
              <div className="grid sm:grid-cols-5 grid-cols-3 gap-4 p-6 bg-main border border-gray-800 rounded-sm">
                ...
                <div className="sm:col-span-2 col-span-3 flex justify-end font-medium text-sm">
                  <Link to={`/watch/${movie.id}`} className='bg-main hover:bg-subMain transitions border-2 border-subMain rounded-full flex-rows gap-4 w-full sm:py-3'>
